...
CountDownTimer.prototype.init = function(setting) {
  this.setting = {
    ...
    selector_number: setting.selector_number
      ? setting.selector_number
      : ".countdown_number",
    selector_outer: setting.selector_outer
      ? setting.selector_outer
      : ".countdown_outer",
    selector_day: setting.selector_day
      ? setting.selector_day
      : ".countdown_day",
    selector_hour: setting.selector_hour
      ? setting.selector_hour
      : ".countdown_hour",
    selector_minute: setting.selector_minute
      ? setting.selector_minute
      : ".countdown_minute",
    selector_second: setting.selector_second
      ? setting.selector_second
      : ".countdown_second",
    selector_separator: setting.selector_separator
      ? setting.selector_separator
      : ".countdown_separator",
    selector_separator_day: setting.selector_separator_day
      ? setting.selector_separator_day
      : ".countdown_separator_day"
  };
...
CountDownTimer.prototype.outputHour = function() {
  var hours = Math.floor(
    (this.state.goal_diff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
  );
  document.querySelector(
    this.setting.selector_hour
  ).innerHTML = this.utilsGetDoubleDigestNumer(hours);
};
// 出力：分
CountDownTimer.prototype.outputMinute = function() {
  var minutes = Math.floor(
    (this.state.goal_diff % (1000 * 60 * 60)) / (1000 * 60)
  );
  document.querySelector(
    this.setting.selector_minute
  ).innerHTML = this.utilsGetDoubleDigestNumer(minutes);
};
// 出力：秒
CountDownTimer.prototype.outputSecond = function() {
  var seconds = Math.floor((this.state.goal_diff % (1000 * 60)) / 1000);
  document.querySelector(
    this.setting.selector_second
  ).innerHTML = this.utilsGetDoubleDigestNumer(seconds);
};
