...
class MemberCollectionTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function test指定した属性のメンバーが追加されていること()
    {
        $memberCollection = new MemberCollection();

        $this->assertThat($memberCollection->count(), $this->equalTo(0), "初期状態のコレクションの要素数が0");

        $memberCollection->addMember('testname', 'testpart', '2015-01-15');
        $this->assertThat($memberCollection->count(), $this->equalTo(1), "追加後のコレクションの要素数が1");
